...
function reservationIdIsCorrectIfPresent(req, res, next) {
  ...
  next({ status: 400, message: `reservation_id '${reservation_id}' should be absent or match url '${reservationId}'.` });
...
function noReservationsInPast(req, res, next) {
  ...
  next({ status: 400, message: `New reservations must be in the future.` })
};
...
function reservationIsDuringBusinessHours(req, res, next) {
  ...
  if (clockTime < 1030 || clockTime > 2130) {
    next({ status: 400, message: `Reservation time '${reservation_time}' must be between 10:30 AM and 9:30 PM.` })
  }
...
async function update(req, res) {
  ...
  const { reservationId } = req.params;
  const data = await reservationsService.update(reservationId, updatedReservation);
...
async function list(req, res) {
  ...
  // console.log(date)
  const test = await reservationsService.list()
  // console.log(test)
  if (date) {
    const data = await reservationsService.listByDate(date);
    res.json({ data });
  } else if (viewDate) {
    const data = await reservationsService.listByDate(viewDate);
    res.json({ data });
